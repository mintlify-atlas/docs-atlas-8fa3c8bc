...
<CardGroup cols={2}>
  ...
  <Card title="Multiple Interfaces" icon="layer-group">
    Submit tasks via Telegram bot or interactive REPL. Both feed the same Supabase task queue.
  </Card>
...
graph TD
    ...
    D -->|Log events| B
    C -->|Notify on completion| F[Telegram]
```

1. **Submit a task** — via Telegram or REPL
...
5. **Results delivered** — sent back to original source (Telegram chat or REPL)
